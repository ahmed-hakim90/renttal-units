...
export function SettingsForm({ settings, locale, canEdit }: { settings: Setting[]; locale: string; canEdit: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg space-y-4 rounded-2xl border border-border bg-card p-6">
      ...
      <Input name="company_name_en" label={t('companyNameEn')} defaultValue={companyValue?.en ?? ''} disabled={!canEdit} />
      <Input name="company_name_ar" label={t('companyNameAr')} defaultValue={companyValue?.ar ?? ''} disabled={!canEdit} />
      <Input name="payment_terms" label={t('defaultPaymentTerms')} type="number" defaultValue={String(termsSetting?.value ?? 30)} disabled={!canEdit} />
      <Input name="grace_days" label={t('overdueGraceDays')} type="number" defaultValue={String(graceSetting?.value ?? 7)} disabled={!canEdit} />
      {canEdit && <Button type="submit">{tc('save')}</Button>}
    </form>
  );
}
